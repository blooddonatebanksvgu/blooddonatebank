...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if (in_array($action, ['approve', 'reject'])) {
        ...
        // Get request details
        $requestQuery = mysqli_query($conn, "SELECT * FROM blood_request WHERE id = $id");
        $request = mysqli_fetch_assoc($requestQuery);
        
        if ($request) {
            if ($action === 'approve') {
                // Check if enough stock is available
                $stockQuery = mysqli_query($conn, "SELECT SUM(quantity_ml) as total FROM blood_stock WHERE blood_group_id = {$request['blood_group_id']}");
                $stock = mysqli_fetch_assoc($stockQuery);
                
                if ($stock['total'] < $request['quantity_ml']) {
                    setFlashMessage('danger', 'Insufficient blood stock to approve this request.');
                    header("Location: blood_requests.php");
                    exit();
                }
                
                // Deduct from stock (from any blood bank with stock)
                $remaining = $request['quantity_ml'];
                $stocksQuery = mysqli_query($conn, "SELECT * FROM blood_stock WHERE blood_group_id = {$request['blood_group_id']} AND quantity_ml > 0 ORDER BY quantity_ml DESC");
                while ($stockRow = mysqli_fetch_assoc($stocksQuery)) {
                    if ($remaining <= 0) break;
                    
                    $deduct = min($stockRow['quantity_ml'], $remaining);
                    mysqli_query($conn, "UPDATE blood_stock SET quantity_ml = quantity_ml - $deduct WHERE id = {$stockRow['id']}");
                    $remaining -= $deduct;
                }
            }
...
$requestsQuery = "SELECT br.*, p.user_id, u.name as patient_name, u.phone, bg.group_name, bb.name as blood_bank_name 
...
if ($statusFilter) {
    $requestsQuery .= " WHERE br.status = '$statusFilter'";
}
...
<div class="card">
    ...
    <div class="card-body">
        <form method="GET" action="" class="d-flex gap-2 align-items-center flex-wrap">
            ...
            <a href="blood_requests.php" class="btn btn-secondary">
...
<div class="card">
    <div class="card-header">
        <h3><i class="fas fa-notes-medical"></i> All Blood Requests</h3>
    ...
    <div class="card-body">
        ...
        <?php else: ?>
            <div class="empty-state">
                ...
                <p>Blood requests will appear here when patients request blood.</p>
            </div>
        <?php endif; ?>
    </div>
</div>
